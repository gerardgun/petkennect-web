...
const { selectors, types } = employeeRoleServiceTypeDetailDuck
  ...
  try {
    ...
    yield put({
      payload: {
        form: {
          ...detail.form,
          employee_role_options: employee_roles.map(({ id, name }) => ({
            text : name,
            value: id
          })),
          service_type_options: service_types.map(({ id, name }) => ({
            text : name,
            value: id
          }))
        }
...
function* deleteItem({ id, employee_role_id }) {
  try {
    yield put({ type: types.DELETE_PENDING })

    yield call(Delete, `employees-roles/${employee_role_id}/service-types/${id}/`)
...
function* post({ payload }) {
  try {
    ...
    const result = yield call(Post, `employees-roles/${payload.employee_role_id}/service-types/`, {
...
function* _put({ payload }) {
  try {
    ...
    const result = yield call(Patch, `employees-roles/${payload.employee_role_id}/service-types/${payload.id}/`, {
      max_reservations_per_day: payload.max_reservations_per_day,
      service_variations      : payload.reservation_type_ids
    })
