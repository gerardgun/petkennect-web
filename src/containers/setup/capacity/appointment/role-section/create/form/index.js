...
const EmployeeRoleServiceTypeForm = props => {
  const {
  ...
  useEffect(() => {
    if(editing) {
      initialize({
        id                      : detail.item.id,
        employee_role_id        : detail.item.role_id,
      ...
      dispatch(
        employeeRoleServiceTypeDetailDuck.creators.edit()
      )
    } else {
      dispatch(
        employeeRoleServiceTypeDetailDuck.creators.create()
      )
    }
  }, [])
  ...
  const _handleServiceTypeChange = serviceTypeId => {
    dispatch(
      employeeRoleServiceTypeDetailDuck.creators.createGetReservationTypes({
  ...
  const _handleSubmit = (values) => {
    if(editing)
      return dispatch(employeeRoleServiceTypeDetailDuck.creators.put(values))
        .then(_handleClose)
        .catch(parseResponseError)
    else
      return dispatch(employeeRoleServiceTypeDetailDuck.creators.post(values))
        .then(_handleClose)
        .catch(parseResponseError)
  }
  ...
  return (
    ...
    <Form id='employee-role-service-type' onReset={reset} onSubmit={handleSubmit(_handleSubmit)}>
      ...
      <Form.Group widths='equal'>
        <Field
          ...
          options={detail.form.employee_role_options}
...
export default reduxForm({
  ...
  validate: values => {
    const schema = {
      employee_role_id        : yup.mixed().required('Role is required'),
