...
const ProductFamilyShow = (props) => {
  ...
  const _handleSubmit = values => {
    ...
    return props.put({ id: productFamilyDetail.item.id, ...values })
...
export default compose(
  connect(
    ({ ...state }) => {
      ...
      return {
        initialValues: { ...productFamilyDetail.item, categories: categories.items.map((item)=>{
          return (productFamilyDetail.item.categories && productFamilyDetail.item.categories.filter(function(id) {
            return id == item.id}).length > 0 ? true : false
          )
        }) },
        productFamilyDetail: productFamilyDetail,
        category           : categories
      }
    },
    {
      getCategories       : categoryDuck.creators.get,
      getProductFamilies  : productFamilyDetailDuck.creators.get,
      resetItem           : productFamilyDetailDuck.creators.resetItem,
      resetProductFamilies: productFamilyDuck.creators.resetItem,
      setItem             : productFamilyDetailDuck.creators.setItem,
