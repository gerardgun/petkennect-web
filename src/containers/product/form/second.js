...
const ProductFormSecond = props => {
  ...
  useEffect(() => {
    ...
    props.getProductVariations(productFamilyDetail.item.id)
    if(productFamilyDetail.item.family)
      props.getProductFamilies(productFamilyDetail.item.family)
  }, [])
  ...
  const _handleAttributeValueChange = (e, { item_id, attribute_id, product_variation, checked }) => {
    ...
    if(checked) {
      ...
      return props.postProductVariation({ product: productFamilyDetail.item.id, attributes })
    }
    else {
        ...
        <>
          <Segment className='p40'>
            ...
            <div className='div_attribute'>
              {
                productFamilyDetail.item.family_attributes && productFamilyDetail.item.family_attributes.map((item) => (
                  <>
                    <Field
                      checked={true}
                      className='pl8'
