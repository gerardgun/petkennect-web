...
const GroomingPackageForm = props => {
  const {
    groomingPackageDetail,
  ...
  const _handleSubmit = values => {
    if(isUpdating)
      return props.put({ id: groomingPackageDetail.item.id, ...values })
        .then(_handleClose)
  ...
  const isOpened = useMemo(() => getIsOpened(groomingPackageDetail.mode), [ groomingPackageDetail.mode ])
  const isUpdating = Boolean(groomingPackageDetail.item.id)
...
export default compose(
  ...
  connect(
    state => {
      const groomingPackageDetail = groomingPackageDetailDuck.selectors.detail(state)

      return {
        groomingPackageDetail,
        initialValues: groomingPackageDetail.item
      }
    },
    {
      post     : groomingPackageDetailDuck.creators.post,
      put      : groomingPackageDetailDuck.creators.put,
      resetItem: groomingPackageDetailDuck.creators.resetItem
    }
  ),
  reduxForm({
    form              : 'grooming-new-package-form',
