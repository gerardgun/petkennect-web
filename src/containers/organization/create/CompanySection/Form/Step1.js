...
const CompanyForm = props => {
  ...
  useEffect(() => {
    if(companyDetail.mode === 'CREATE')
      if(organization.items.length === 0) props.getOrganizations()
  }, [ companyDetail.mode ])
  ...
  return (
    <>
      {
        isUpdating && (<Header as='h2' className='segment-content-header'>Update Company</Header>)
      }
      ...
      <Form onReset={reset} onSubmit={handleSubmit(_handleSubmit)}>
        ...
        <Form.Group className='form-modal-actions' widths='equal'>
          <Form.Field>
            ...
            <Button
              color='teal'
              content={isUpdating ? 'Save changes' : 'Continue'}
              disabled={submitting}
              loading={submitting}/>
          </Form.Field>
        </Form.Group>
