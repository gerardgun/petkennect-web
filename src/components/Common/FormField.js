...
const FormField = (props) => {
  ...
  const getComputedInput = () => {
    ...
    if(props.type === 'file') delete computed.value
  ...
  const _handleBlur = (e, data = {}) => {
    ...
    if(typeof data.value !== 'undefined') value = data.value
    else value = e
  ...
  const _handleChange = (e, { type, checked, value }) => {
    if(type === 'checkbox') return onChange(checked)
  ...
  return (
    <Form.Field className={className} error={!!hasError} required={required}>
      {' '}
      {/* update for help icon with label    */}
      {!!label
            ...
            <Popup
              ...
              trigger={
                <Icon
                  name='info circle'
                  size='large'
                  style={{ 'padding-left': '.5rem', 'margin-bottom': '.3rem' }}/>
              }/>
          </label>
        ) : (
          <label>{label}</label>
